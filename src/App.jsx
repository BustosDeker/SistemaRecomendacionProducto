...
export default function App() {
  ...
  const actualizarRecomendaciones = useCallback(() => {
    if (historialCompras.length > 0) {
      ...
      redNeuronal.entrenar(historialCompras);
      ...
      const nuevasRecomendaciones = redNeuronal.recomendar(historialCompras, numRecomendaciones);
  ...
  useEffect(() => {
    if (usuarioActual) {
      const historial = JSON.parse(localStorage.getItem(`historial_${usuarioActual}`) || '[]');
  ...
  const handleLogin = () => {
    ...
    if (modoRegistro) {
      ...
      if (!usuarios[formLogin.username] || usuarios[formLogin.username] !== formLogin.password) {
  ...
  const comprarProducto = (producto) => {
    const nuevoHistorial = [...historialCompras, { 
      ...producto, 
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {historialCompras.length > 0 && vistaActual !== 'recomendaciones' && (
        <div className="fixed bottom-4 right-4 bg-purple-600 text-white px-4 py-2 rounded-full shadow-lg flex items-center space-x-2 cursor-pointer hover:bg-purple-700 transition"
